...
OUT_DIR = Path("/root/autodl-tmp/utplm/outputs/evaluations/b1diaga2strengthv4")
OUT_DIR.mkdir(parents=True, exist_ok=True)

rows: List[Dict] = []
for p in ROOT.glob("b1diaga2strength_*/summary.json"):
    obj = json.loads(p.read_text(encoding="utf-8"))
    rows.append(obj)

if not rows:
    raise SystemExit("No b1diaga2strength summary.json found.")

df = pd.DataFrame(rows)
all_csv = OUT_DIR / "b1diaga2strengthallrunsummaries.csv"
agg_csv = OUT_DIR / "b1diaga2strengthgroupaggregate.csv"
