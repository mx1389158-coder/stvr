...
OUT_DIR = Path("/root/autodl-tmp/utplm/outputs/evaluations/b1_diag_progmargin_v3")
OUT_DIR.mkdir(parents=True, exist_ok=True)

rows: List[Dict] = []
for p in ROOT.glob("b1diagmarginv3_*/summary.json"):
    obj = json.loads(p.read_text(encoding="utf-8"))
    rows.append(obj)

if not rows:
    raise SystemExit("No b1diagmarginv3 summary.json found.")

df = pd.DataFrame(rows)
all_csv = OUT_DIR / "b1diagmarginv3allrunsummaries.csv"
agg_csv = OUT_DIR / "b1diagmarginv3groupaggregate.csv"
