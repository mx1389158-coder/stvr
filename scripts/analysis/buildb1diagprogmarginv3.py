...
OUT_DIR = PROJECT_ROOT / "data" / "processed" / "b1diagprogmarginv3" / TAG
...
SCRIPT_VERSION = "build_b1diagprogmarginv3"
...
def main() -> None:
    ...
    if target_n <= 0:
        raise RuntimeError(f"No usable progmargin-v3 pairs. high={high_raw}, low={low_raw}")
    ...
    high_rows = build_pref_rows(high_prog, "dpo_high_progmargin_v3")
    low_rows = build_pref_rows(low_prog, "dpo_low_progmargin_v3")

    out_high = OUT_DIR / "b1diagv3prefhighprogmargin.jsonl"
    out_low = OUT_DIR / "b1diagv3preflowprogmargin.jsonl"
    out_manifest = OUT_DIR / "b1diagv3manifest.json"
    ...
    manifest = {
        ...
        "counts": {
            "high_prog_raw_after_margin_v3": int(high_raw),
            "low_prog_raw_after_margin_v3": int(low_raw),
            "target_pair_count_balanced": int(target_n),
            "pref_high_progmargin_v3_rows": int(len(high_rows)),
            "pref_low_progmargin_v3_rows": int(len(low_rows)),
        },
        "outputs": {
            "pref_high_progmargin_v3": str(out_high),
            "pref_low_progmargin_v3": str(out_low),
            "manifest": str(out_manifest),
        },
        "notes": [
            "Diagnostic-3 fixes negative source and overlap, and increases high/low margin by using margin-v3 pools.",
        ],
    }
